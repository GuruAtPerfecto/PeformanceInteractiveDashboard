...
public class ReportiumExportUtils {
    ...
    private static final String CQL_NAME = System.getProperty("CLOUD_NAME"); // TODO put your Continuous Quality Lab name here

    // TODO put your security token here
    // See http://developers.perfectomobile.com/display/PD/DigitalZoom+Reporting+Public+API on how to obtain a Security Token
    private static final String PERFECTO_SECURITY_TOKEN = System.getProperty("PERFECTO_SECURITY_TOKEN");
    ...
    public static JsonObject retrieveTestExecutions(String executionId) throws URISyntaxException, IOException {
        ...
        uriBuilder.addParameter("startExecutionTime[0]", "1692988200000");
        uriBuilder.addParameter("endExecutionTime[0]", "1693062737000");
     ...
     * @throws IOException
     */
    public static JsonObject retrieveTestExecutionsPagewise(int page, String urlParameters) throws URISyntaxException, IOException {

        URIBuilder uriBuilder = new URIBuilder(REPORTING_SERVER_URL + "/export/api/v3/test-executions");

        String regexAlphanumeric = "\\d+"; // Matches one or more word characters (alphanumeric + underscore)

        if(!Pattern.matches(regexAlphanumeric, urlParameters)){
            //passed url parameter is driverExecutionID
            uriBuilder.addParameter("externalId[0]", urlParameters);
        }
        else{ //passed url parameter is startExecutionTime in UnixTimeStamp format
            uriBuilder.addParameter("startExecutionTime[0]", urlParameters);
            uriBuilder.addParameter("_page", String.valueOf(page));
            uriBuilder.addParameter("status[0]", "PASSED");
        }
    ...
    public static JsonObject getTotalExecution(String startExecutionTime) throws URISyntaxException, IOException {
        URIBuilder uriBuilder = new URIBuilder(REPORTING_SERVER_URL + "/test-execution-management-webapp/rest/v1/test-execution-management/status-groups/search");

        HttpPost getTestExecutions = new HttpPost(uriBuilder.build());

        String jsonPayload = "{\"filter\":{\"fieldNameToSearchFilter\":{},\"fields\":{\"startExecutionTime\":" + startExecutionTime + "},\"excludedFields\":{}},\"groupBy\":[\"status\"]}";

        StringEntity entity = new StringEntity(jsonPayload);
        entity.setContentType("application/json");
        getTestExecutions.setEntity(entity);



        JsonObject testExecutionsJson;
        addDefaultRequestHeaders(getTestExecutions);
        HttpClient httpClient = HttpClientBuilder.create().build();

        HttpResponse getExecutionsResponse = httpClient.execute(getTestExecutions);
        ...
        try (InputStreamReader inputStreamReader = new InputStreamReader(getExecutionsResponse.getEntity().getContent())) {
            ...
            try {
                testExecutionsJson = gson.fromJson(response, JsonObject.class);
